...
class ReplayBuffer:
    """FIFO experience replay buffer with save/load support."""
...
def sac(env_name=TurboRaceEnv, actor_critic=SAC_TurboCore.MLPActorCritic, ac_kwargs=dict(),
    ...
    print(f"Starting SAC training for {epochs} epochs (resume from {start_epoch})")

    for t in range(start_epoch * steps_per_epoch, total_env_steps):
        ...
        if (t + 1) % steps_per_epoch == 0:
            ...
            print(f"Epoch {epoch}/{epochs} | Q Loss: {current_q_loss:.3f} | "
                  f"Pi Loss: {current_pi_loss:.3f} | Avg Ep Return: {avg_ep_ret:.3f}")
            ...
            checkpoint = {
                ...
                "epoch": epoch,
                "replay_buffer": replay_buffer.state_dict()  # <--- save replay buffer
            }
...
if __name__ == '__main__':
    sac(seed=0,
        ...
        max_ep_len=35000,
        reset_noise=0,
        model_save_path='/Users/venky/Documents/Projects/DRL_TurboEngine/DRL_TurboEngineV1/SAC_TrainedWeights/SAC_TrainedWeights_TR1.pth',
        resume_path='/Users/venky/Documents/Projects/DRL_TurboEngine/DRL_TurboEngineV1/SAC_TrainedWeights/SAC_TrainedWeights_TR1_epoch11.pth'
    )
